...
CIS_COUNTRY_CODES = {
    # базовый “СНГ”, плюс Украина
    "AM", "AZ", "BY", "KZ", "KG", "MD", "RU", "TJ", "TM", "UZ",
    # часто встречается в подобных списках
    "GE",
    # Украина
    "UA", "UKR",
}

# ISO numeric (часто бывает в geoip): UA=804, RU=643, BY=112, KZ=398, KG=417, MD=498, AM=051, AZ=031, TJ=762, TM=795, UZ=860, GE=268
CIS_NUMERIC_CODES = {"804", "643", "112", "398", "417", "498", "051", "31", "031", "762", "795", "860", "268"}
...
def _truthy(v) -> bool:
    ...
    if isinstance(v, (int, float)):
        return v == 1
    s = str(v).strip().lower()
    return s in ("1", "true", "yes", "y", "ok", "cis", "да")
...
def is_country_allowed_from_api(data: dict | None) -> bool:
    """
    Разрешаем:
    - если API говорит is_cis=true (в любом формате)
    - ИЛИ если страна определяется как Украина
    - ИЛИ если country_code в списке CIS_COUNTRY_CODES (в т.ч. UA)
    """
    if not data:
        return False

    # 1) разные варианты ключей "cis"
    cis_val = (
        data.get("is_cis")
        if "is_cis" in data
        else data.get("cis", data.get("isCis", data.get("isCIS", None)))
    )
    if _truthy(cis_val):
        return True

    # 2) country_code / cc / code / iso / numeric
    cc = _norm_str(data.get("country_code") or data.get("cc") or data.get("code") or data.get("iso") or data.get("country") if len(_norm_str(data.get("country"))) in (2, 3) else "").upper()
    ...
    # numeric code
    cnum = _norm_str(
        data.get("country_code_numeric")
        or data.get("country_numeric")
        or data.get("numeric")
        or data.get("country_code_num")
        or ""
    )
    if cnum and cnum in CIS_NUMERIC_CODES:
        return True

    # иногда country_code может быть числом/строкой "804"
    if cc and cc.isdigit() and cc in CIS_NUMERIC_CODES:
        return True
    ...
    cname = _norm_str(
        ...
        or data.get("geo_country")
        or data.get("location")
        or ""
    ).lower()

    # Украина — максимально широко
    if "ukrain" in cname or "укра" in cname:
        return True

    # если вдруг API отдаёт страну как "UA Ukraine" / "Ukraine (UA)" и т.п.
    if " ua" in cname or "(ua" in cname or "ua)" in cname:
        if "ukrain" in cname or "укра" in cname:
            return True
...
def run_one_time_cis_unblock_migration():
    """
    ✅ Одноразово “разблокирует” тех, кто раньше отлетел по причине НЕ СНГ
    (cis_checked=1, cis_ok=0), чтобы они могли пройти проверку заново.
    Запускается 1 раз при старте и помечается в config.
    """
    key = "cis_unblock_once_v1"
    try:
        ...
        # сбрасываем “не СНГ” результаты, чтобы люди могли перепройти проверку
        cursor.execute(
            """
            UPDATE users
            SET cis_ok=1, cis_checked=0
            WHERE cis_checked=1 AND cis_ok=0
            """
        )

        cursor.execute(
            "INSERT OR REPLACE INTO config(key, value) VALUES(?, ?)",
            (key, now_kyiv().isoformat()),
        )
        conn.commit()

        _qwarn("[INFO] One-time CIS unblock migration applied.")
    except Exception as e:
        _qwarn(f"[WARN] One-time CIS unblock migration failed: {type(e).__name__}")

# запускаем миграцию сразу после инициализации БД
run_one_time_cis_unblock_migration()

async def ensure_cis_access(user_id: int, carrier) -> bool:
    """
    Одноразовая проверка через сайт:
    - если cis_checked=1 и cis_ok=1 -> пускаем
    - если cis_checked=1 и cis_ok=0 -> блок (с кнопкой перепроверки)
    ...
    if cis_checked and cis_ok_val == 0:
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="🔄 Пройти проверку заново", callback_data="cis_recheck")]
            ]
        )
        await safe_send_message(
            chat_id,
            "🚫 Бот доступен только для пользователей из стран СНГ (Украина тоже подходит).\n"
            "Если вы считаете, что это ошибка — нажмите «🔄 Пройти проверку заново».",
            reply_markup=kb,
        )
        return False
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="✅ Я прошёл проверку", callback_data="cis_check_done")],
        ]
    )
    await safe_send_message(
        chat_id,
...
@dp.callback_query(lambda c: c.data == "cis_recheck")
async def cis_recheck_callback(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    try:
        cursor.execute(
            "UPDATE users SET cis_ok=1, cis_checked=0 WHERE user_id=?",
            (user_id,),
        )
        conn.commit()
    except Exception as e:
        _qwarn(f"[WARN] cis_recheck update failed: {type(e).__name__}")
    ...
    await callback.answer("Ок! Пройдите проверку заново.", show_alert=True)
    await ensure_cis_access(user_id, callback)
...
async def cis_check_done_callback(callback: types.CallbackQuery):
    ...
    if cis_ok_val == 1:
        ...
        await safe_send_message(
            user_id,
            "🚫 По результатам проверки ваш IP не относится к странам СНГ (Украина допускается).",
        )
...
async def evaluate_risks_for_referrer(referrer_id: int) -> str:
    ...
    cis_ok_count = 0            # ✅ сколько “с СНГ/Украины” по IP
    non_cis_or_unknown = 0      # ❌ не СНГ или нет данных
    ...
    body = (
        ...
        f"с СНГ (по IP): {cis_ok_count} ({pct(cis_ok_count)}%)\n"
        f"не СНГ / нет данных по IP: {non_cis_or_unknown} ({pct(non_cis_or_unknown)}%)\n"
...
async def main_menu_handler(message: types.Message):
    ...
    if text in menu_buttons:
        ...
        if text == "Заработать звезды🌟":
            ...
            info_text = (
                "<b>📚 Схема работы бота</b>\n\n"
                "1) /start → проверка страны (СНГ/Украина) → выбор пола → проверка подписки на спонсоров.\n"
                "2) После успешной подписки открывается главное меню.\n\n"
                "<b>⭐️ Как заработать</b>\n"
                "• Приглашайте друзей по своей реферальной ссылке (кнопка «Заработать звезды🌟»).\n"
                f"• За каждого друга, который зайдёт по ссылке и подпишется на спонсоров: <b>+{REFERRAL_REWARD}.0⭐️</b>\n"
                f"• Каждые <b>{REFERRAL_BONUS_EVERY}</b> приглашённых — бонус: <b>+{REFERRAL_BONUS_AMOUNT}.0⭐️</b>\n\n"
                "<b>🪜 Схема уровней (инфо)</b>\n"
                "• 1 уровень: 10%\n"
                "  Пример: реферал 1 уровня заработал 100 → тебе начисляется +10\n"
                "• 2 уровень: 5%\n"
                "  Пример: реферал 2 уровня заработал 100 → тебе начисляется +5\n"
                "• 3 уровень: 2%\n"
                "  Пример: реферал 3 уровня заработал 100 → тебе начисляется +2\n\n"
                "<b>💳 Вывод</b>\n"
                "• Доступные суммы: <b>15⭐️ / 25⭐️ / 50⭐️ / 100⭐️</b>\n"
                "• Перед подачей заявки нужно написать администратору <b>@aaR1ss</b>\n\n"
                "<b>🔒 Важно</b>\n"
                "• Если аккаунт заблокирован админом — профиль обнуляется и все заявки на вывод удаляются."
            )
            await safe_answer_message(
